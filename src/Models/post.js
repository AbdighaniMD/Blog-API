...
postSchema.pre(/^find/, function (next) {
    // add view count as virtual field
    postSchema.virtual('viewCount').get(function(){
        const post = this;
        return post.numViews.length;
    });

    // add view  like count as virtual field
    postSchema.virtual('likeCount').get(function(){
        const post = this;
        return post.likes.length;
    });

    // add view  dislike  count as virtual field
    ...
    postSchema.virtual("daysAgo").get(function () {
        ...
        const daysAgo = Math.floor((Date.now() - date) / 86400000); // 1day = 86400000 as milliseconds 
        return daysAgo === 0
        ? "Today"
        : daysAgo === 1
        ? "Yesterday"
